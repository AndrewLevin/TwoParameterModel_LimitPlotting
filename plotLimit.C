...
std::string param1_label = "";
std::string param2_label = "";

//======================================================================

TString par2latex(const TString& parname)
{
  assert(param1_label != "" && param2_label != "");

  //  if (parname.EqualTo("param1") )  return "F_{T1}/#lambda^{4} (100 TeV^{-4})";
  //  if (parname.EqualTo("param2") ) return "F_{T2}/#lambda^{4} (100 TeV^{-4})";
  if (parname == "param1" ) return param1_label;
  if (parname == "param2" ) return param2_label;
...
{
  if (parname == "param1" )  return 0.0001;
  if (parname == "param2" ) return 0.0001;
				  ...
				  map<string,TGraph2D *>& m_graphs)
{
  ...
  for( size_t i=0; i<fnames.size(); i++) {
    ...
    for (size_t j = 0, n = t->GetEntries(); j < n; ++j) {
      ...
      std::cout << par1 << " " << par2 << " " << deltaNLL << std::endl;

      //a point at (0,0) seems to be always added by combine, even if it is not in the grid (at least for the expected limits)
      //such a single point not on the grid can cause problems with the delaunay triangulation that is used to find the contours
      //therefore, we exclude this point
      if(par1 == 0 && par2 == 0) 
	{
	  std::cout << "skipping (0,0) points" << std::endl; 
	  continue;
	}
		     ...
		     map<string,TList *>& m_contours)
{
  ...
  for (size_t i=0; i<keys.size(); i++) {
    ...
    if (gr2d && (gr2d->GetN() > 0)) {
      std::cout << "prttt 1"  << std::endl;
      std::cout << "clev = " << clev << std::endl;
      gr2d->GetHistogram()->SetContour(1, &clev);
      //gr2d->GetHistogram()->SetContour(1); 
      std::cout << "prttt 2"  << std::endl;
      //canv->cd(i+1);
      cout << "drawing... " << endl;
	    ...
	    TLegend *legend)
{
  ...
  std::cout << "interpolate 3" << std::endl;
  ...
  std::cout << "interpolate 4" << std::endl;
...
void plotLimit(std::string expected_filename, std::string PARAM1_LABEL, std::string PARAM2_LABEL)
{
  ...
  //fnames.push_back("/home/anlevin/2d_limit_fw/CMSSW_6_1_1/src/CombinedEWKAnalysis/CommonTools/test/higgsCombineTest.MultiDimFit.mH120.delete_this.root");
